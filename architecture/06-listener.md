...
### ❌ Listener doing many things

```php
final class HandleOrderPlaced implements ShouldQueue
{
    public function handle(OrderPlaced $event): void
    {
        $this->sms->send(...);
...
If the SMS fails, do we want to retry the whole thing? Now we double-send emails. One Listener per reaction.

### ❌ Listener calling an Action

```php
public function handle(OrderPlaced $event): void
{
    app(MarkOrderAsHighPriority::class)->handle($event->order);  // ❌
}
```

Listeners are reactions, not orchestrators. If the order needs to be marked high-priority, that decision belongs in the Action that placed the order, or in a Job triggered by the Action.

### ❌ Listener firing further Events

```php
public function handle(OrderPlaced $event): void
{
    $this->sms->send(...);
    event(new SmsSent(...));  // ❌ creates an event chain
}
```

Event chains become impossible to reason about. If you need a downstream reaction, structure it as another Listener on the original Event, not a chain.

### ❌ Sync Listener for slow work

```php
final class SendOrderConfirmationSms  // no ShouldQueue
{
    public function handle(OrderPlaced $event): void
    {
        $this->sms->send(...);  // blocks HTTP response
    }
}
```

The user waits for the SMS API. Default to `ShouldQueue` unless you have a specific reason not to.
